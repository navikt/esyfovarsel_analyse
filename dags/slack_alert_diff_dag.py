...
def get_esyfo_n_rows_yesterday():
    ...
    project = 'team***'
    sql = """ ... """  # fyll inn riktig SQL
    # Her bruker du pandas_gbq.read_gbq(sql, project_id=project)
    # returnerer antall rader som int
    '''
    pass
...
def varsel_status(**context):
    diff = 5  # Hardkodet differanse for test, f.eks uke 18/19
    ret = 'varsling' if diff > 0 else 'stop_task'
...
def sjekk_og_send_slack(**context):
    ...
    if status == 'varsling':
        return f"TEST VARSEL: Differanse er {diff} rader denne uken."
    else:
        return None

with DAG(
    'overvakning_diff_test',
    schedule_interval=None,  # Ingen automatisk kjøring
